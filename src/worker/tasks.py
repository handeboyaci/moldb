...
def _process_chunk_job(smiles_list: list[str], starting_line: int):
  ...
  # Store results in Redis for aggregation
  job = rq.get_current_job()
...
def _aggregate_results_job(parent_job_id: str):
  ...
  for i, (job_id, result_json) in enumerate(results_hash.items()):
    result = json.loads(result_json)
    total_ingested += result["successfully_ingested"]
    total_failed += result["failed_count"]
    all_errors.extend(result["errors"])
    job.meta["progress"] = (i + 1) / len(results_hash) * 100
    job.save_meta()
