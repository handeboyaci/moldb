...
def wait_for_job(job_id, queue):
  """
  Fetches a job from the given queue.
  In a test environment with a synchronous queue, the job is expected
  to be already finished or failed.
  """
  job = queue.fetch_job(job_id)
  if job is None:
    raise ValueError(f"Job {job_id} not found in the queue.")
  return job


@pytest.mark.usefixtures("seed_molecules")
def test_search_molecules_no_filters(client, context_aware_task_queue):
  response = client.get("/api/v1/search")
  assert response.status_code == 202
  job = wait_for_job(response.json()["job_id"], context_aware_task_queue)
...
@pytest.mark.usefixtures("seed_molecules")
def test_search_molecules_with_min_mol_weight(client, context_aware_task_queue):
...
@pytest.mark.usefixtures("seed_molecules")
def test_search_molecules_with_max_mol_weight(client, context_aware_task_queue):
...
@pytest.mark.usefixtures("seed_molecules")
def test_search_molecules_with_min_and_max_mol_weight(client, context_aware_task_queue):
...
@pytest.mark.usefixtures("seed_molecules")
def test_find_similar_molecules(client, context_aware_task_queue):
...
@pytest.mark.usefixtures("seed_molecules")
def test_substructure_search(client, context_aware_task_queue):
...
@pytest.mark.usefixtures("seed_molecules")
def test_search_molecules_with_all_filters(client, context_aware_task_queue):
  response = client.get(
...
@pytest.mark.usefixtures("seed_molecules")
def test_search_molecules_with_chemical_identifiers(client, context_aware_task_queue):
  # Search by InChI
  response = client.get(
...
@pytest.mark.usefixtures("seed_molecules")
def test_find_similar_molecules_caching(client, context_aware_task_queue):
  # Clear cache for this test
  context_aware_task_queue.connection.flushdb()
  ...
  job1 = wait_for_job(response1.json()["job_id"], context_aware_task_queue)
  ...
  job2 = wait_for_job(response2.json()["job_id"], context_aware_task_queue)
  ...
  job3 = wait_for_job(response3.json()["job_id"], context_aware_task_queue)
...
def test_ingest_molecules_success(
  mock_open_file, mock_stat, client, context_aware_task_queue
):
  ...
  job = wait_for_job(job_id, context_aware_task_queue)
  assert job.get_status() == "finished"


@patch("src.worker.tasks.Path.stat", side_effect=FileNotFoundError)
def test_ingest_molecules_file_not_found(mock_stat, client, context_aware_task_queue):
